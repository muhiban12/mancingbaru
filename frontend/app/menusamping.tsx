...
export default function SideDrawer({ visible, onClose }: SideDrawerProps) {
  ...
  // Logic State: Untuk menampung data user dari backend/database
  ...
  // Ambil data dari storage setiap kali drawer dibuka
  useEffect(() => {
    if (visible) {
      loadUserData();
  ...
  const loadUserData = async () => {
    try {
      ...
      if (userJson) {
        ...
        setUserData({
          // Menyesuaikan dengan kolom 'nama_lengkap' atau 'nama' di database apps_pancingin
          nama: user.nama_lengkap || user.nama || "User",
          // Jika di DB ada kolom role (owner/user), kita buat kondisinya di sini
          role: user.role === "owner" ? "Owner Member" : "Angler Member",
  ...
  const handleLogout = async () => {
    try {
      await AsyncStorage.multiRemove(["token", "user"]); // Hapus semua sesi
      onClose();
    ...
    <Modal transparent visible={visible} onRequestClose={onClose}>
      <View style={styles.overlay}>
        <Animated.View style={[styles.drawer, { transform: [{ translateX }] }]}>
          {/* Header user info - SEKARANG DINAMIS */}
          ...
          {/* Promo card - HANYA MUNCUL JIKA BUKAN OWNER */}
          {!userData.isOwner && (
          ...
          {/* Footer logout - SEKARANG DINAMIS */}
